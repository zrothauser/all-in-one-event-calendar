require( [
         ...
         function ( $, domReady, gmaps_helper, AI1EC_UTILS, page, ai1ec_config, calendrical_functions ) {
	"use strict"; // jshint ;_;
	// Globalize jQuery
	window.jQuery = $;
	window.$ = $;
	...
	describe( "Add new event page", function() {
		...
		describe( "Test suite for the 'Event location' Box", function() {
			...
			it( "Should show an error after the latitude field if the latitude is not valid and blur is triggered", function() {
				...
				expect( $( "#ai1ec_latitude" ).next() ).toHaveText( ai1ec_config.error_message_not_valid_lat );
			} );
			it( "Should show an error after the longitude field if the longitude is not valid and blur is triggered", function() {
				...
				expect( $( "#ai1ec_longitude" ).next() ).toHaveText( ai1ec_config.error_message_not_valid_long );
			} );
			...
			it( "Should show some suggestions when entering a value in the address field and format the result after clicking on it. It also fills some hidden fields.", function() {
				$( '#ai1ec_address' ).val( 'Via Andrea del Sarto, 9 20129' );
				...
				runs( function() {
					...
					expect( $( 'div.ac_results li' ).length ).toEqual( 1 );
					// click on the first
					$( 'div.ac_results li:first' ).click();
					// The address fields get this value
					expect( $( '#ai1ec_address' ) ).toHaveValue(
						'Via Andrea del Sarto, 9, 20129 Milan, Italy'
					);
					// Check the hidden fields
					expect( $( '#ai1ec_country' ) ).toHaveValue( 'Italy' );
					expect( $( '#ai1ec_postal_code' ) ).toHaveValue( '20129' );
					expect( $( '#ai1ec_province' ) ).toHaveValue( 'Lombardy' );
					expect( $( '#ai1ec_city' ) ).toHaveValue( 'Milan' );
				} );
			} );
			...
			it( "Should show a warning when latitude or longitude are not both set and the publish button is pressed", function() {
				...
				$( '#publish' ).click();
				expect( $( "#ai1ec_longitude" ).next() ).toBe( 'div.ai1ec-error' );
				expect( $( "#ai1ec_longitude" ).next() ).toHaveText( ai1ec_config.error_message_not_entered_long );
			} );
		} );
		...
		describe( "Test suite for the 'event date and time' box ", function() {
			...
			it( "Clicking on the all-day-event checkbox should hide the time inputs and disable the instantaneous checkbox, while unchecking it should redisplay things", function() {
				$( '#ai1ec_all_day_event' ).click();
				expect( $( '#ai1ec_instant_event' ) ).toBeDisabled();
				waits( 500 );
				runs( function() {
					expect( $( '#ai1ec_start-time-input' ) ).toBeHidden();
					expect( $( '#ai1ec_end-time-input' ) ).toBeHidden();
					$( '#ai1ec_all_day_event' ).click();
				} );
				waits( 500 );
				runs( function() {
					expect( $( '#ai1ec_instant_event' ) ).not.toBeDisabled();
					expect( $( '#ai1ec_start-time-input' ) ).not.toBeHidden();
					expect( $( '#ai1ec_end-time-input' ) ).not.toBeHidden();
				} );
			} );
			it( "Clicking on the instantaneus checkbox should hide the end dat inputs and disable the all-day-event checkbox, while unchecking it should redisplay things", function() {
				$( '#ai1ec_instant_event' ).click();
				expect( $( '#ai1ec_all_day_event' ) ).toBeDisabled();
				waits( 500 );
				runs( function() {
					expect( $( '#ai1ec_end-date-input' ) ).toBeHidden();
					expect( $( '#ai1ec_end-time-input' ) ).toBeHidden();
					$( '#ai1ec_instant_event' ).click();
				} );
				waits( 500 );
				runs( function() {
					expect( $( '#ai1ec_all_day_event' ) ).not.toBeDisabled();
					expect( $( '#ai1ec_end-date-input' ) ).not.toBeHidden();
					expect( $( '#ai1ec_end-time-input' ) ).not.toBeHidden();
				} );
			} );
			it( "should initialize the start end end date based on the current time ( as passed by wordpress ) and formatted - This test might need adjustment because of your timezone", function() {
				...
				expect( $( '#ai1ec_start-time-input' ) ).toHaveValue( '3:30pm' );
				expect( $( '#ai1ec_end-time-input' ) ).toHaveValue( '4:30pm' );
