require( [
         ...
         function ( $, domReady, event_handlers, utility_functions, ajax_handlers, AI1EC_UTILS, page, ai1ec_config ) {
	"use strict"; // jshint ;_;
	// Globalize jQuery
	window.jQuery = $;
	window.$ = $;
	...
	describe( "Facebook Feeds page", function() {
		...
		describe( "Test suite for the cancel_element_and_reorder_other function", function() {
			...
			it( "Should show a message if the last container was deleted", function() {
				...
				expect( $( '.ai1ec-facebook-items[data-type=page] .no_subscription' ) ).toHaveText( ai1ec_config.no_more_subscription );
			} );
		} );
	} );
