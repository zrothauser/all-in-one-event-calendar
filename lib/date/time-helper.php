...
class Ai1ec_Time_Utility extends Ai1ec_Base {
	 ...
	 * local time zone.
	 *
	 * @param int    thisstamp Timestamp to use for evaluation
	 * @param string this      Timezone name, from which to calculate offset
	 *
	...
	public function get_gmt_offset_expr(
		...
		$timezone = $this->get_gmt_offset( $timestamp, $zone );
		$timezone = sprintf(
	 ...
	 * round over to it).
	 *
	 * @param int thisstamp Timestamp at which difference must be evaluated
	 *
	...
	public function gmt_to_local( $timestamp ) {
		return $timestamp + $this->get_local_offset_from_gmt( $timestamp );
	}
	...
	public function local_to_gmt( $timestamp ) {
		$gmtized    = $timestamp - $this->get_local_offset_from_gmt(
			$timestamp
		);
		// {$gmtized} used intentionally as TZs are set on GMT basis
		$dst_offset = $this->dst_difference( $gmtized );
	 ...
	 *
	 * @param string thiste_tz Remote timezone, to calculate offset from
	 * @param string thisin_tz Origin timezone, to calculate offset to
	 * @param int    thisstamp Reference timestamp, at which offset shall be
	...
	public function __construct( Ai1ec_Registry_Object $registry ) {
		...
		$this->_timezone_offsets = $this->_registry->get( 'cache.memory' );
		$this->_dst_differences  = $this->_registry->get( 'cache.memory' );
