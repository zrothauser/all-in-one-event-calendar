...
class Ai1ec_Factory_Strategy extends Ai1ec_Base {
	 ...
	 *
	 * @param string $cache_folders Cache directory to use
	...
	public function create_cache_strategy_instance(
		...
		if ( true !== $skip_small_bits && Ai1ec_Cache_Strategy_Apc::is_available() ) {
			...
			NULL !== $cache_folders &&
			$cache_folder = $this->_get_writable_cache_dir( $cache_folders )
		) {
	...
	public function create_persistence_context(
		...
		return new Ai1ec_Persistence_Context(
			$key_for_persistance,
			$this->create_cache_strategy_instance( $cache_folders, $skip_small_bits )
		);
	}
	 ...
	 * 
	 * @param array $cache_folders
	 * @return boolean|string
	 */
	protected function _get_writable_cache_dir( $cache_folders ) {
		$writable_folder = false;
		foreach ( $cache_folders as $cache_folder ) {
			if ( $this->_is_cache_dir_writable( $cache_folder['path'] ) ) {
				$writable_folder = $cache_folder;
				break;
			}
		}
